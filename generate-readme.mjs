...
const buildStats = ({ entries, meals, daily }) => {
...
const buildRecent = ({ meals, daily }) => {
  ...
  const recentDays = last(daily, 7).reverse();
  const recentMeals = last(meals, 12).reverse();

  return `### Last 7 logged days
...
const main = async () => {
  ...
  nextReadme = replaceSection(
    nextReadme,
    "stats",
    `${buildStats({ entries, meals, daily })}\n\n_Last updated: ${new Date().toLocaleDateString("en-US", { dateStyle: "long" })}_`,
  );
